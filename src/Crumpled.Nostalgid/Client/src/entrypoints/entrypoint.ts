...
const LEGACY_HASH_ROUTE_REGEX = /^#\/?(?:content\/content|media\/media)\/edit\/(\d+)(?:\/|$)/i;

function redirectLegacyHashRoute() {
  const { hash, pathname } = window.location;
  if (!hash || !pathname.toLowerCase().startsWith("/umbraco")) {
    return;
  }

  const match = hash.match(LEGACY_HASH_ROUTE_REGEX);
  if (!match) {
    return;
  }

  const legacyId = Number.parseInt(match[1], 10);
  if (!Number.isFinite(legacyId) || legacyId <= 0) {
    return;
  }

  window.location.replace(`/umbraco/nostalgid/${legacyId}`);
}
